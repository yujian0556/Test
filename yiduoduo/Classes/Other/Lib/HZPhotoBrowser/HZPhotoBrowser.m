...
- (void)showPhotoBrowser
{
    ...
    CGRect rect = [self.sourceImagesContainerView convertRect:sourceView.frame toView:parentView];
    // 减去偏移量
    ...
    rect.origin.y =  rect.origin.y - contentOffsetY;
...
- (void)hidePhotoBrowser:(UITapGestureRecognizer *)recognizer
{
    ...
    UIView *sourceView = self.sourceImagesContainerView.subviews[self.currentImageIndex];
    UIView *parentView = [self getParsentView:sourceView];
    CGRect targetTemp = [self.sourceImagesContainerView convertRect:sourceView.frame toView:parentView];
    ...
    targetTemp.origin.y =  targetTemp.origin.y - contentOffsetY;
